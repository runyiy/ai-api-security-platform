...
**v0.1.1 · reviewed design / N1 adopted · local fake-only implementation pending independent review.** Reviewed `302b1e581f4e0e559a0f16f924d1ca64ea985adb` was integrated through [PR #152](https://github.com/runyiy/ai-api-security-platform/pull/152). PR #153 established the adopted N1 runtime prerequisite. The subsequent [implementation, T/X/Y acceptance map and local validation](research-ai-w2-runtime-validation.md#w2-implementation-and-acceptance) cover the concrete protocol below; hosted acceptance of this new local candidate has not run.

[B1–B8 adoption](research-ai-budget-contract.md#w2-design-adoption-record) remains binding. **D** denotes reviewed concrete design detail. **N1** denotes the separately adopted fake observer mechanism, including deployment-wide acceptance closure and explicit recovery before reopening; it is not a decision attributed to the earlier B1–B8 adoption. Neither decision needs repeat adoption. All account/credentials, data/retention/egress, spending, deployment and live-provider gates remain separate and closed. Source observations below are tied to the original contract base `e953ee08cb32e0ba9de0f3d196e7b5e82b9fe727`.
...
The local implementation also uses two parent-only persistence records under the same strict metadata bounds: `CallEvidence {key_digest:Hash, evidence_digest:Hash}` binds the separate immutable core file to the journal; `WriterFence {deployment_ref:Id, recovery_decision_id:Id, authority_epoch:Id, acceptance_generation:Gen}` fences old database writers before explicit reopening. They use `ra-w2-call-evidence/1` and `ra-w2-writer-fence/1` respectively and grant no child capability.

### 3.2 Signatures, ownership and errors

All signatures below are internal contract obligations; their implementation is in [the W2 package](../backend/app/ai/w2/), with no public route. `ReadContext` is a frozen per-operation value containing exact Scope, process epoch, monotonic/UTC deadlines and cancellation ID, with no call yet. `RecoveryContext` contains the same scoped identifiers, an explicit synthetic recovery decision ID, a newer owner generation, a separate bounded operation deadline ≤30s, and no send permission. Preparation and recovery may never restart the old call deadline. `CallRuntime` is a newly constructed per-call facade with immutable RunContext and explicit trusted Clock/CancelReader/Authority/Coordination/Observer dependencies; no singleton, thread-local implicit authority or reused “current request” object.
...
The adopted PostgreSQL budget authority is implemented by the additive [W2 migration](../backend/alembic/versions/7ba5dce4a936_add_research_ai_budget.py) and internal budget store. Logical relations:
...
| New W2 configuration, decision/projection/source-certificate registry, budget policy, cancellation and recovery writers | [W2 registry, policy, cancellation and recovery writers](../backend/app/ai/w2/); W1 Configuration/Registry remain frozen values | Mandatory G on every replacement/revoke/disable/version/owner/retention change; §5.3 barrier also applies without a successor admission; fixed current version pointers plus immutable history. Key rotation means version references only; no secret storage/credential reads added |
...
**N1 — ADOPTED for fake-only tests; local implementation awaits independent review:** for fake-only process tests, use an independently owned parent test harness as permit authority and fake endpoint, with bounded IPC and an append-only, fsync-before-ack journal in its owned temporary directory. The adapter child process cannot mint permits, alter journal files or append endpoint acceptance records. A separate read-only test observer retains endpoint counts and journal high-water marks outside both database result/accounting callbacks. N1 also includes the §5.3 deployment-wide generation barrier shared by every invalidating writer and endpoint acceptance, its acknowledged close-before-commit ordering, and explicit recovery before reopening. The later User adoption covers this targeted N1 mechanism; B1–B8 stay adopted. It is not an adopted live service, worker, Redis component, deployment or account-evidence channel. The evaluated alternative of a test-only independently owned PostgreSQL observer server would add resources and different shared-failure assumptions; it is not selected by this record.
...
Pre-code review of the concrete B1/B8 design and separate N1 adoption are complete. Implementation review must still verify writer coverage, lock order and every acceptance group above. Later runtime tests need independently owned verified disposable PostgreSQL and fake processes. The historical documentation checks in [PR #152](https://github.com/runyiy/ai-api-security-platform/pull/152) validated links, strict JSON, record definitions, necessity/transition/accounting examples and T1–T12 mapping; they are not proof of runtime transaction safety, process isolation or live-provider bounds.

## P1 guard-loss lifecycle correction

The historical v0.1.0 protocol missed an invalidator committing after G loss with **no successor**. Reviewed v0.1.1 binds invalidation and acceptance to N1's shared A boundary, requires durable deployment-wide closure before invalidation commit, pins tickets before qualification, and keeps uncertain acknowledgements/transactions closed or unresolved. Interfaces, writer inventory, X6, Y1–Y5 and T6/T8/T11 retain that corrected order.

The [reviewed correction](https://github.com/runyiy/ai-api-security-platform/commit/302b1e581f4e0e559a0f16f924d1ca64ea985adb) records six Y trace cases and all 70 order-preserving model interleavings, detecting the no-barrier and split-check/write counterexamples. These document/model checks assume A and durable evidence; they do not prove atomic IPC, fsync, process isolation or runtime fencing. N1 was subsequently adopted; the historical pending-N1 stop point is retired. The prerequisite alone did not implement the protocol. The later local candidate and its T/X/Y observations are recorded in [runtime validation](research-ai-w2-runtime-validation.md#w2-implementation-and-acceptance).
