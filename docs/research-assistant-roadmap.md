...
> **RA-01/W1 当前实施记录（2026-09-09）：** [产品与能力契约 v0.1.0](research-assistant-product-contract.md) 已形成文档交付，状态为 **IMPLEMENTED / PENDING_INDEPENDENT_REVIEW**；精确起点 `cec06c5c5f497e0f277b29d8316fa9b7cf726725`。RA-01 处于进行中，未获 reviewer-PASS、未 COMPLETE；W2/W3 尚未开始。下文 NOT_STARTED / NOT_AUTHORIZED、Issue #132 和 push 流程保留为规划历史；当前交接的持续授权适用于已有顺序工作包，但本次仅 W1：实施、验证、本地 commit 后停止，由独立 Review Project 审查并 push 精确批准的 HEAD。不重开 Issue #132 / merged PR #133，不扩大架构决策、公网执行或费用授权。
...
| --- | --- | --- | --- | --- |
| RA-01 | 可执行的产品/评测契约及 ADR 规格 | 计划审阅后单独批准首个任务 | 冻结 oracle、阈值及必要 ADR | IN_PROGRESS；W1 文档已实施、待独立审查；W2/W3 未开始 |
...
- **有序工作包：** (1) **RA-01/W1：** 冻结产品契约、真实调用图和支持形态；每项声明都有源码、限制和输出证据。 [W1 契约 v0.1.0](research-assistant-product-contract.md) 已实施，待独立审查（非 reviewer-PASS）。(2) **RA-01/W2：** 定义合成开发集与隔离保留集、确定性时钟、可执行评分规则、结果 schema 和第 7 节阈值；提供样例输入/输出及最小评分演示。(3) **RA-01/W3：** 审查第 8 节 ADR 提案；批准 RA-02 所需数据决策，并记录其余决策的审批检查点。
...
- **有序工作包：** (1) **RA-02/W1：** 输入项目规则、许可和预算并校验来源；缺少许可时阻止执行准备。(2) **RA-02/W2：** 受限离线解析、项目隔离和最小化持久化，不产生网络副作用。(3) **RA-02/W3：** 明确身份/Resource/slot 提议、业务事实输入和人工凭据更新；缺少事实进入 NEEDS_INPUT。为 RA-04 提供会话检查事实，本阶段不发送 Target 请求。
