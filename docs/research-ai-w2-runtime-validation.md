...
**LOCAL W2 IMPLEMENTATION CANDIDATE / INDEPENDENT REVIEW PENDING.** The fake-only implementation starts at `27ee5948af8ea3569dd5f934610356a6a32afc26` on `codex/ra-05-w2-implementation`. Its implementation and local evidence are mapped below. No push, PR, merge, deployment, provider/Target call or paid usage is part of this work.
...
| T7 | [Process tests](../backend/tests/ai/test_w2_processes.py) kill actual sandbox children at reserve/admission, marker, permit, endpoint acceptance, usage and settlement boundaries. Surviving PostgreSQL, journal and witness evidence determines zero/known/held liability; no automatic resend. |
...
| T9 | Identical events make no postings; conflicting final totals, reused IDs and a changed core for a consumed question append conflict evidence and pause. Explicit correction adjusts only the delta. Trusted late usage never revives cancellation or display. |
...
| X5 | Append-only settlement/correction: `test_x5_duplicate_conflict_and_delta_correction` in [accounting tests](../backend/tests/ai/test_w2_accounting.py) compares SQL before/after identical events, conflicting totals and an explicitly authorized delta. |
...
The final complete local run passed **3,963 tests in 1,901.97s (31m 41s)**, with 62 reported collection/deprecation warnings and no skips or expected failures. It includes all 146 added acceptance cases and existing backend regressions. Migration to `7ba5dce4a936`, migration round-trips/metadata checks, actual W2 privilege negatives and the complete/disjoint collection check passed. Python was **3.12.3**, PostgreSQL **16**; CI workflow, runtimes, shards, aggregation and the CI-only AppArmor policy were unchanged.

| Final run evidence | Value |
| --- | --- |
