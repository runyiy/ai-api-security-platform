...
from app.db.models.research_task import TABLES as _TASK_TABLES
TASK_TABLES = {m.__tablename__ for m in _TASK_TABLES}
...
def snapshot(*, legacy=False):
    with engine.connect() as db:
        return {t.name: list(db.execute(select(t).order_by(*t.primary_key.columns)).mappings())
                for t in Base.metadata.sorted_tables if not legacy or t.name not in (TASK_TABLES | AI_BUDGET_TABLES | NEW_TABLES | OBSERVATION_TABLES | KNOWLEDGE_TABLES | RULE_VALIDATION_TABLES | INTENT_TABLES | VERIFICATION_TABLES | {"research_subject_versions"})}
