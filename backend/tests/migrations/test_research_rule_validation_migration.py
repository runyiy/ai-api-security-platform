...
from tests.research_intake_fixtures import VERIFICATION_TABLES, snapshot, RULE_VALIDATION_TABLES, INTENT_TABLES
...
def previous():
    ...
    with engine.connect() as db:
        return {t.name: list(db.execute(select(*[c for c in t.columns if c.name != 'hold_generation']).order_by(*t.primary_key.columns)).mappings())
            for t in Base.metadata.sorted_tables if t.name not in TASK_TABLES | AI_BUDGET_TABLES | VERIFICATION_TABLES | RULE_VALIDATION_TABLES | INTENT_TABLES}
