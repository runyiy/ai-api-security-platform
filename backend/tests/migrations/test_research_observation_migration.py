...
def existing_snapshot():
    with engine.connect() as db:
        return {t.name: list(db.execute(select(t).order_by(*t.primary_key.columns)).mappings())
                for t in Base.metadata.sorted_tables if t.name not in TABLES | RULE_VALIDATION_TABLES | KNOWLEDGE_TABLES | {"research_subject_versions"}}
...
def test_populated_rollback_protected_without_loss(observation_context):
    ...
    with engine.connect() as db:
        assert MigrationContext.configure(db).get_current_revision() == "a1c3e5f7b9d0"
